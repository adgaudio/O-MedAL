...
Method_type = 1                                 # 0:Baseline accuracy computation (Entire dataset) 
                                                # 1: Random sampling  2:Max Entropy measure  3:Our method
Initial_dataset_size = 20                       # Initial dataset size to sample using ORB descriptors
No_of_images_to_sample = 5                      # No of images to sample each AL iteration, common for all method types
...
Create_initial_dataset = True                   # True:  Performs ORB descriptor technique (unless Load_descriptors = True) to create initial training and oracle sets from overall available dataset
...
Epochs = 80                                # no of epochs to train model for each AL iteration
...
Img_resol = 300                           # Input image resolution (Height == Width assumed) 
Availble_training_data_size = 700
Availble_test_data_size = 200
No_classes = 2
	...
	if Method_type==0:
		...
		if Create_initial_dataset==True:
			...
			if K.image_data_format()=='channels_last':        
				...
				X_test = np.reshape(X_train,(len(X_test),Img_resol,Img_resol,3))
				...
				X_test = np.reshape(X_train,(len(X_test),3,Img_resol,Img_resol))
